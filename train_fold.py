...
def train(epoch, dataloader):
	...
	for data in dataloader:
		...
		all_loss += F.nll_loss(model(data, all_loss).log_softmax(dim=-1), data.y)
		all_loss.backward()
...
if __name__=="__main__":
	...
	#loss_fn = get_obj_from_str(optimizer_config['loss_fn'])(weight=torch.as_tensor(train_dset.weights).to(device))
	...
	if epochs_to_train > 0:
		...
		for epoch in range(begin_epoch, end_epoch):
			...
			tracker.append(valid_acc, test_accs, optimizer.param_groups[0]['lr'])
			print(f"Epoch: {epoch+1:03d}, Validation: {valid_acc:.4f}, Test: ", '\t'.join([f'{fmax:.4f}' for fmax in test_accs]))
		...
		print(f"Best epoch: {best_epoch:03d}, Validation: {best_valid:.4f}, Test: ", '\t'.join([f'{fmax:.4f}' for fmax in best_test]))

		# save checkpoint
		best_model_path = os.path.join(project_root, f"model_ep{best_epoch:03d}.pt")
