...
if __name__=="__main__":
	...
	if epochs_to_train > 0:
		...
		for epoch in range(begin_epoch, end_epoch):
			...
			test_fmax = [test(dl) for dl in test_loaders]

			tracker.append(valid_fmax, test_fmax, optimizer.param_groups[0]['lr'])
			print(f"Epoch: {epoch+1:03d}, Validation: {valid_fmax:.4f}, Test: ", '\t'.join([f'{fmax:.4f}' for fmax in test_fmax]))
